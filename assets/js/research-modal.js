  ...
  const THEMES = [
    {
      ...
      body: [
        ...
        'My work integrates Landsat thermal imagery, sociodemographic data, and machine-learning models (SAE, ANN, SVM) to surface the populations most exposed, and to predict how those patterns shift under continued urbanisation. This is the thread that grew into my PhD on extreme-heat risk.',
      ],
      bodyZh: [
        '热浪并不会平均地降落在一座城市之上。工薪阶层社区、老旧高层、远离绿地的街区，承担了城市热岛加剧最严重的部分——以及随之而来的最严重的健康代价。',
        '我的工作将 Landsat 热红外影像、社会人口数据与机器学习模型（SAE、ANN、SVM）相结合，识别出最易暴露的人群，并预测在持续城市化进程下这些模式将如何变化。正是这条线索，生长成了我关于极端高温风险的博士研究。',
      ],
      visual: 'uhi',
    },
    {
      ...
      body: [
        ...
        'Recent collaborations have used SHAP-interpretable models on carbon-emission grids across the Pearl River Basin, identifying forests as dominant carbon sinks and construction land as the fastest-growing emission source. Interpretability is not a luxury here; it is what makes results actionable — and it is the methodological core I am now carrying toward remote-sensing AI and foundation models.',
      ],
      bodyZh: [
        '一颗卫星能告诉你一件事，一份普查数据告诉你另一件事。研究真正的工作，是把它们优雅、透明地融合在一起——尊重每一种数据能说与不能说的东西。',
        '近期的合作研究在珠江流域的碳排放栅格上使用了 SHAP 可解释模型，识别出森林是主导性的碳汇，建设用地是增长最快的排放源。在这里，可解释性不是奢侈品，而是让研究结果真正可用的前提——这也是我正带向遥感 AI 与基础模型的方法学内核。',
      ],
      visual: 'ml',
    },
    {
    ...
    },
    {
      ...
      body: [
        ...
        'In Bristol I built U-Safety — a multi-source pipeline pairing crime statistics, traffic data, and spatial features with ML-derived safety indices, then routed student commutes through Dijkstra and A* to minimise exposure. The principle generalises: data infrastructure as policy infrastructure — and equity remains a pillar of the heat-resilience work I do now.',
      ],
      bodyZh: [
        '谁能走到诊所、公园、一个安全的公交站？市中心与外围之间，这样的距离又会如何变化？环境正义研究的成败，正系于对这些问题的回答。',
        '在布里斯托，我搭建了 U-Safety——一条多源数据流水线，将犯罪统计、交通数据与空间特征与机器学习导出的安全指数结合，再以 Dijkstra 与 A* 算法为学生通勤规划最低风险路径。其中的原则可以推广：数据基础设施即政策基础设施——而公平，至今仍是我热浪韧性研究的支柱之一。',
      ],
      visual: 'justice',
    },
  ];
